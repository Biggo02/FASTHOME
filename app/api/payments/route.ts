...
function statusFor(expected: number, paid: number): 'UPCOMING' | 'PAID' | 'PARTIAL' {
  if (paid <= 0) return 'UPCOMING';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error(error);
    return fail('Impossible de récupérer les paiements.', 500);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (expectedAmount <= 0 || paidAmount < 0) return fail('Montants invalides.', 400);
    ...
    await prisma.auditLog.create({ data: { actorId: body.actorId?.trim() || null, action: 'PAYMENT_RECORDED', entity: 'Payment', entityId: payment.id, metadata: { contractId, expectedAmount, paidAmount } } });
    return ok(payment, 201);
  } catch (error) {
    console.error(error);
    return fail(error instanceof Error ? error.message : 'Enregistrement du paiement impossible.', 400);
  }
}
