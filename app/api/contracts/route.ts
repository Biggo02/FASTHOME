...
function contractReference(year: number, sequence: number) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!partyId) return fail('partyId est obligatoire.', 400);
    const contracts = await prisma.contract.findMany({ where: { partyId }, include: { property: { select: { reference: true, title: true, city: true, commune: true, neighborhood: true } }, payments: true }, orderBy: { createdAt: 'desc' } });
    return ok(contracts);
  } catch (error) {
    console.error(error);
    return fail('Impossible de récupérer les contrats.', 500);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error(error);
    return fail(error instanceof Error ? error.message : 'Création des contrats impossible.', 400);
  }
}
